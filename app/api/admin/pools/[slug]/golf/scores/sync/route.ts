...
const missedCutScore =
  ...
  if (!process.env.RAPIDAPI_KEY) {
    redirect(`/admin/pools/${pool.slug}/golf/scores?syncFailed=1`);
  }
  ...
  const activeSource = sources.find((source) => source.isActive);

  if (!activeSource) {
    redirect(`/admin/pools/${pool.slug}/golf/scores?sourceMissing=1`);
  }

  const response = await fetch(activeSource.sourceUrl, {
    method: "GET",
    headers: {
...
if (!matchedGolfer) {
  unmatchedNames.set(normalizedName, name);
  continue;
}

const normalizedStatus = String(row.status ?? "").toLowerCase();
const normalizedDisplayPosition = String(row.displayPosition ?? "").toLowerCase();
...
displayPosition: effectiveDisplayPosition || null,
  ...
  if (values.length > 0) {
    ...
    await db
      ...
      .where(eq(leaderboardSources.id, activeSource.id));
  }

  if (unmatchedNames.size > 0) {
    const encoded = encodeURIComponent(
      JSON.stringify(Array.from(unmatchedNames.values()))
    );

    redirect(
      `/admin/pools/${pool.slug}/golf/aliases?unmatched=${encoded}&matched=${values.length}`
    );
  }
